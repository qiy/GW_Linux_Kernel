...
static struct task_struct *atom_led_detect_thread;
#endif
...
static long _atom_ioctl_core(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    copyRet = copy_from_user(&_atom_buf1, (void *)arg, sizeof(atom_data_buf));
    printk("atom for test ioctl data: atom_flag:%d, atom_data_buf1:%s\n",
            _atom_buf1.atom_flag, _atom_buf1.atom_data_buf1); 

    ptr = strstr(_atom_buf1.atom_data_buf1 , "atom_Led=");
    ptr += strlen("atom_Led=");
    atom_arg = (int)simple_strtol(ptr, NULL, 10);
    printk("_atom_arg = %d\n", atom_arg); 
//		i4RetValue = 1;
		break;
	case ATOM_WRITE_OFF:
    copyRet = copy_from_user(&atom_arg, (void *)arg, sizeof(atom_arg));
		printk("FLASH_IOC_IS_LOW_POWER");
         g_atom_InitFunc-> atom_ioctl(cmd,atom_arg);
//        _atom_en_pin_output_low();
		break;
...
#ifdef led_kthread
	atom_led_detect_thread =
	    kthread_run(atom_led_detect_sw_thread_handler, NULL,
			"CPJ atom_led_detect_sw_workaround");
	if (IS_ERR(atom_led_detect_thread)) {
		printk("[%s]: failed to create atom_led_detect_sw_workaround thread\n", __func__);
	}
#endif
...
#ifdef led_kthread
    if (atom_led_detect_thread)                                                                                                                
        kthread_stop(atom_led_detect_thread);
#endif
