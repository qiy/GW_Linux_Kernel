...
{ 
    ...
    char buf_flag[10] = {5,15,25,35,50,60,75,80,90,0};
...
//	}
    ...
    sleep(2);
    err = ioctl(fd,ATOM_WRITE_ON,&buf_flag[0]);
   sleep(2);
    err = ioctl(fd,ATOM_WRITE_ON,&buf_flag[1]);
    sleep(2);
    err = ioctl(fd,ATOM_WRITE_ON,&buf_flag[2]);
   sleep(2);
    err = ioctl(fd,ATOM_WRITE_ON,&buf_flag[3]);
     sleep(2);
    err = ioctl(fd,ATOM_WRITE_ON,&buf_flag[4]);
   sleep(2);

       err = ioctl(fd,ATOM_WRITE_OFF,&atom_flag_led);
