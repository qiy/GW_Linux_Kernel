...
#define CAMERA_SUPPORT_LENGTH 1024
#define CAMERA_USING_LENGTHE 256
char cameralist_supported[CAMERA_SUPPORT_LENGTH] = {0};
char cameralist_using[CAMERA_USING_LENGTHE] = {0};
...
UINT32 kdGetSensorInitFuncList(ACDK_KD_SENSOR_INIT_FUNCTION_STRUCT **ppSensorList)
{
	...
	*ppSensorList = &kdSensorList[0];
	...
	if(i == 0)
	{
		listsize = sizeof(kdSensorList)/sizeof(kdSensorList[0]);
		memset(cameralist_supported, 0, sizeof(cameralist_supported));
		for(i=0;i<listsize;i++)
		{
			if(kdSensorList[i].SensorId == 0x00)
				break;
			strcat(cameralist_supported,kdSensorList[i].drvname);
			strcat(cameralist_supported,"	");
		}
	}
		...
		for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
			if (DUAL_CAMERA_NONE_SENSOR != g_invokeSocketIdx[i]) {
				...
				if (sensorID == 0) {	/* not implement this feature ID */
					...
					static int foundCameraNum=0;
					#define NumOfCameras 2
					if(foundCameraNum < NumOfCameras)	//如果已经找到了指定数量的camera，则不会再记录camera信息
					{
						foundCameraNum++;

						strcat(cameralist_using,g_invokeSensorNameStr[i]);
						if(g_invokeSocketIdx[0] == 0x02)
							strcat(cameralist_using,"(front)");
						else if(g_invokeSocketIdx[0] == 0x01)
							strcat(cameralist_using,"(back)");
						strcat(cameralist_using,"	");			
					}
...
static ssize_t cameralist_using_proc_read(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	//int len = 0;
	char *p = page;
...
static ssize_t cameralist_supported_proc_read(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	//int len = 0;
	char *p = page;
    if (*ppos)  // CMD call again
	...
	strcat(cameralist_supported,"\0");
	p += sprintf(p,"%s\n",cameralist_supported);
...
static const struct file_operations cameralist_using_proc_ops = {
    .owner = THIS_MODULE,
    .read = cameralist_using_proc_read,
};

static const struct file_operations cameralist_supported_proc_ops = {
    .owner = THIS_MODULE,
    .read = cameralist_supported_proc_read,
};

static void CAMERA_list_proc(void)
{
    struct proc_dir_entry *cameralist_using_prEntry = NULL;	
    struct proc_dir_entry *cameralist_supported_prEntry = NULL;	
	cameralist_using_prEntry = proc_create("cameralist_using", 0, NULL, &cameralist_using_proc_ops);
    if (cameralist_using_prEntry) {
        PK_ERR("add /proc/cameralist_using entry fail \n");
    }

	cameralist_supported_prEntry = proc_create("cameralist_supported", 0, NULL, &cameralist_supported_proc_ops);
    if (cameralist_supported_prEntry) {
        PK_ERR("add /proc/cameralist_supported entry fail \n");
    }
}
